...
class ZaloAPI(object):
	...
	def _listen(self, delay=0, thread=False):
		HasRead = set()
	...
	def _listen_test(self, delay=0, thread=False):
		HasRead = set()
		...
		while not self._condition.is_set():
			...
			for groupId in Groups:
				groupmsgs = self.getRecentGroup(groupId).groupMsgs
	...
	def startListening(self, delay=1, test=False, thread=False, background=False):
		...
		Args:
			...
			background (bool): Background listening mode (Default: False)
		...
		if background:
			[
				pool.submit(self._listen, delay, thread)
				if not test else
				pool.submit(self._listen_test, delay, thread)
			]
			self.listening = True
		
		else:
			[
				self._listen(delay, thread)
				if not test else
				self._listen_test(delay, thread)
			]
	...
	def listen(self, delay=1, test=False, thread=False, background=False):
		...
		"""
		self.onListening()
		self.startListening(delay, test, thread, background)
		if background:
	...
	def onLoggingIn(self, phone=None):
		...
		"""
		print("\033[1;32mLogin {}...".format(phone))
	...
	def onLoggedIn(self, phone=None):
		...
		Args:
			phone: The phone number of the client
		"""
		print("\033[1;34mĐăng Nhập Số Điện Thoại {} Thành Công.".format(phone))
	
	def onListening(self):
		"""Called when the client is listening."""
		print("\033[1;36mStatus Bot Of Nguyễn Ngọc Coder")
