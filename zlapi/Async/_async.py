...
class ZaloAPI(object):
	...
	async def _getGroupBoardList(self, board_type, page, count, last_id, last_type, groupId):
		...
		if results:
			...
			results = Group.fromDict(results, self._undefinded) if results.get("error_code") == 0 else results
	...
	async def kickUsersFromGroup(self, members, groupId):
	...
	async def send(self, message, thread_id, thread_type=ThreadType.USER, mark_message=None):
		...
		if message.mention:
			return await self.sendMentionMessage(message, thread_id)
		else:
			return await self.sendMessage(message, thread_id, thread_type, mark_message)
	
	async def sendMessage(self, message, thread_id, thread_type, mark_message=None):
	...
	async def replyMessage(self, message, replyMsg, thread_id, thread_type):
	...
	async def sendMentionMessage(self, message, groupId):
	...
	async def sendReaction(self, msgId, clientMsgId, reactionIcon, thread_id, thread_type, reactionType=75, msgType=1):
	...
	async def sendRemoteFile(self, fileUrl, thread_id, thread_type, fileName="default", fileSize=None, extension="vrxx"):
	...
	async def sendLocalImage(self, imagePath, thread_id, thread_type, width=2560, height=2560, message=None):
	...
	async def sendSticker(self, stickerId, cateId, thread_id, thread_type):
	...
	async def _listen_test(self, delay=1):
		HasRead = set()
		Groups = [groupId for groupId in (await self.fetchAllGroups()).gridVerMap]
		while not self._condition.is_set():
			ListenTime = int((time.time() - 10) * 1000)
			...
			for groupId in Groups:
				messages = await self.getRecentGroup(groupId)
				try:
